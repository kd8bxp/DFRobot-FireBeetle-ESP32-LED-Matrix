...
void decodePeopleJson() {
 ...
 void displayPeopleLED() {
  display.clearScreen();
  char temp2[50];
  sprintf(temp2, "There are %d people are in space. ", number);
 ...
 for (int i=0;i<number; i++) {
  ...
  //char *temp2 = (char)name[i] + ", " + (char)craft[i]+" ";
 //display.print(name[i] + ", " + craft[i]+" ",30);
 const char *temp1;
 temp1=name[i].c_str();
 display.print(temp1,30);
 display.print(" on board ",30);
 const char *temp2;
 temp2=craft[i].c_str();
 display.print(temp2,30);
 display.print(" ",30);
...
void displayPassSerial() {
 ...
 for (int i=0;i<count; i++) {
  ...
  //char *temp = risetime[i]+" [" + (char)duration[i]+" m] ";
 ...
 char temp[15];
 sprintf(temp, "%d.%02d",(int)duration[i],abs((int)(duration[i]*100)%100));
